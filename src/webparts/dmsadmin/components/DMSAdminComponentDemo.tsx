...
//   console.log(props , " here is my prop")
 ...
 data()
  ...
  //aman 27/2/26
    ...
    const [siteCollections, setSiteCollections] = useState<any[]>([]); //Ritik 03/04/2026 - state to hold site collections for dropdown in manage user permission component
...
const handleReturnToMain = (Name:any) => {
  ...
  useEffect(() => {
    ...
    // Ritik 03/04/2026 added for fetching the site collection from MasterSiteCollection List
  useEffect(() => {
    getcurrentuseremail();
...
const fetchSiteCollections = async () => {
  try {
    const items = await sp.web.lists.getByTitle("MasterSiteCollection").items.select("Title", "SiteURL", "Id", "IsActive").filter("IsActive eq 1")();
                  ...
                  {activeComponent === 'Manage user permission' && (
                    <div>
                      ...
                      {/* <Manageuserpermissioninonego/> */}
                        ...
                        <Manageuserpermissioninonego sp={activeSp} siteCollections={[]} context={context} />
                    </div>
